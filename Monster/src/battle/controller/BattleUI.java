...
public class BattleUI {
    ...
    private BattleCommand selectPlayerCommand(BattleMonster player, Battle battle) {
    	...
    	while (true) {
            ...
            System.out.println("3. チームを見る"); // 追加
 ...
 // -----------------------------
 // 両チーム表示ユーティリティ（BattleUI に追加）
 // -----------------------------
 private void showBothParties(Battle battle) {
     showPartyStatus(battle, true, "あなたのチーム");
     showPartyStatus(battle, false, "相手のチーム");
 }

 private void showPartyStatus(Battle battle, boolean isPlayerParty, String label) {
     List<BattleMonster> party = isPlayerParty ? battle.getPlayerParty() : battle.getEnemyParty();
     String ownerName = isPlayerParty ? battle.getPlayer().getName() : battle.getEnemy().getName();

     System.out.println();
     System.out.println("=== " + label + " (" + ownerName + ") ===");

     for (int i = 0; i < party.size(); i++) {
         BattleMonster bm = party.get(i);
         if (bm == null || bm.getInstance() == null) {
             System.out.println((i + 1) + ". (不明なモンスター)");
             continue;
         }

         // 基本情報
         String nick = bm.getInstance().getNickname();
         int curHp = bm.getCurrentHp();
         int maxHp = bm.getMaxHp();
         int level = safeInt(() -> bm.getInstance().getLevel(), -1);
         boolean fainted = bm.isFainted();
         String faintMark = fainted ? "（倒れている）" : "";

         // タイプ（Species 側に getTypes() がある想定）
         String types = "";
         try {
             var typesList = bm.getInstance().getSpecies().getTypes(); // List<Type> か List<String>
             if (typesList != null && !typesList.isEmpty()) {
                 // Type オブジェクトなら toString() か getName() を使う
                 StringBuilder sb = new StringBuilder();
                 for (int t = 0; t < typesList.size(); t++) {
                     Object tt = typesList.get(t);
                     sb.append(tt == null ? "?" : tt.toString());
                     if (t < typesList.size() - 1) sb.append("/");
                 }
                 types = sb.toString();
             }
         } catch (Exception ignored) {}

         // 実数値（Stats）表示（存在するフィールドだけ表示）
         String statsLine = "";
         try {
             var s = bm.getInstance().getActualStats(); // Stats 型
             if (s != null) {
                 statsLine = "HP:" + s.getHp()
                         + " ATK:" + s.getAttack()
                         + " DEF:" + s.getDefense()
                         + " SpA:" + s.getSpAttack()
                         + " SpD:" + s.getSpDefense()
         ...
         // ランク補正（Stage）
         String stageLine = "Stages: ATK" + bm.getAtkStage()
                 + " DEF" + bm.getDefStage()
                 + " SpA" + bm.getSpAStage()
                 + " SpD" + bm.getSpDStage()
                 + " SPD" + bm.getSpeStage()
         ...
         // 表示
         System.out.println((i + 1) + ". " + nick + " " + faintMark + "  Lv:" + (level >= 0 ? level : "?")
                 + "  HP: " + curHp + "/" + maxHp + "  Type: " + (types.isEmpty() ? "?" : types));
         if (!statsLine.isEmpty()) System.out.println("    " + statsLine);
         System.out.println("    " + stageLine);
         ...
         try {
             ...
             if (moves != null && !moves.isEmpty()) {
                 ...
                 for (int m = 0; m < moves.size(); m++) {
                     Move mv = moves.get(m);
                     String mvName = mv == null ? "技" + (m + 1) : mv.getName();
                     String mvType = "";
                     try { mvType = mv.getType() == null ? "" : mv.getType().toString(); } catch (Exception ignored) {}
                     String mvCat = "";
                     try { mvCat = mv.getCategory() == null ? "" : mv.getCategory().toString(); } catch (Exception ignored) {}
                     int power = 0;
                     try { power = mv.getPower(); } catch (Exception ignored) {}
                     int acc = 0;
                     try { acc = mv.getAccuracy(); } catch (Exception ignored) {}
                     System.out.println("      - " + (m + 1) + ". " + mvName + " [" + mvType + "] " + mvCat + "  威力:" + power + " 命中:" + acc + "%");
                 }
             } else {
                 System.out.println("    技：なし");
             }
         } catch (Exception ignored) {}
...
//-----------------------------
//安全呼び出しヘルパー（BattleUI に追加）
//-----------------------------
private interface ThrowingSupplier<T> { T get() throws Exception; }

private <T> T safe(ThrowingSupplier<T> s, T fallback) {
  try { T v = s.get(); return v == null ? fallback : v; } catch (Exception e) { return fallback; }
}

private int safeInt(ThrowingSupplier<Integer> s, int fallback) {
  try { Integer v = s.get(); return v == null ? fallback : v; } catch (Exception e) { return fallback; }
}
